...
400 FORMAT(a1)
  ...
  OPEN(7,file='mult-2.pro',status='old')
  OPEN(8,file='mult10.pro',status='old')
  OPEN(9,file='multall.pro',status='unknown')
...
13   READ(6,500,end=99) a1,tmp,c
     ...
     IF(ipc.EQ.0) THEN
        WRITE(*,*)'error reading ufitobs.cat; asteroid ', tmp
        STOP
     ELSE
        name2=tmp(1:ipc-1)
        lr=LEN(name2)
...
99 CLOSE (7)
  CLOSE (8)
  CLOSE (9)
  OPEN(7,file='mult-2.sig',status='old')
  OPEN(8,file='mult10.sig',status='old')
  OPEN(9,file='multall.sig',status='unknown')
...
98 CLOSE (7)
  CLOSE (8)
  CLOSE (9)
  OPEN(7,file='mult-2.del',status='old')
  OPEN(8,file='mult10.del',status='old')
  OPEN(9,file='multall.del',status='unknown')
